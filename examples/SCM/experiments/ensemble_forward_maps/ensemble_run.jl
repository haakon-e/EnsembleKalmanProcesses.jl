...
function run_ensemble(param_names, param_values, N_ens)
    ...
    path_suffix = "lognormal"
    outdir_root = joinpath(pwd(), "output")
    n_params = length(param_names)
    outdir_path = joinpath(outdir_root, "results_ens_p$(n_params)_e$(N_ens)_$(scm_name)_$path_suffix")
    ...
    # Save output
    # namefile
    tmp_namefile_path = namelist_directory(raw_output_dirs[1], scm_name)
    save_namefile_path = namelist_directory(outdir_path, scm_name)
    cp(tmp_namefile_path, save_namefile_path)
    # Stats file
    for (ens_i, raw_output_dir) in enumerate(raw_output_dirs)
        tmp_data_path = joinpath(raw_output_dir, "stats/Stats.$scm_name.nc")
